    ...
    <div class="d-flex align-items-center col-12 pt-2">
        ...
        <div class="d-flex align-items-center col-3">
            <div class="col-4">
                ...
                if ($key != 0) {
                ?>
                    <button class="btn btn-outline-secondary" onclick="sw2(<?= $value['id']; ?>,<?= $sk[$key - 1]['id']; ?>,'re_skills','w')">往上</button>
                <?php
                }
                if ($key != count($sk) - 1) {
                ?>
                    <button class="btn btn-outline-secondary" onclick="sw2(<?= $value['id']; ?>,<?= $sk[$key + 1]['id']; ?>,'re_skills','w')">往下</button>
                <?php
                }
                ?>
            </div>
            <div class="col-4"><input type="checkbox" name="show" onclick="api2('display2','re_skills',<?= $value['id']; ?>,'w')" <?= ($value['sh'] == 1) ? "checked" : ""; ?>><?= ($value['sh'] == 1) ? "顯示" : "隱藏"; ?></div>
            <div class="col-4"><button class="btn btn-outline-danger" onclick="api2('del','re_skills',<?= $value['id']; ?>,'w')">刪除</button></div>
        </div>
    </div>
<?php
}
