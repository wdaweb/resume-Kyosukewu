        ...
        <div class="col-12 mb-5">
            ...
            <div class="d-flex align-items-center col-12">
                ...
                <div class="col-1">
                    ...
                    if ($key != 0) {
                    ?>
                        <button class="btn btn-outline-secondary" onclick="sw2(<?= $value['id']; ?>,<?= $pro[$key - 1]['id']; ?>,'re_pro','pg')">往上</button>
                    <?php
                    }
                    if ($key != count($pro) - 1) {
                    ?>
                        <button class="btn btn-outline-secondary" onclick="sw2(<?= $value['id']; ?>,<?= $pro[$key + 1]['id']; ?>,'re_pro','pg')">往下</button>
                    <?php
                    }
                    ?>
                </div>
                <div class="col-1"><input type="checkbox" name="show" onclick="api2('display2','re_pro',<?= $value['id']; ?>,'pg')" <?= ($value['sh'] == 1) ? "checked" : ""; ?>><?= ($value['sh'] == 1) ? "顯示" : "隱藏"; ?></div>
                <div class="col-1"><button class="btn btn-outline-danger" onclick="api2('del','re_pro',<?= $value['id']; ?>,'pg')">刪除</button></div>
            </div>
        </div>
    </div>
<?php
